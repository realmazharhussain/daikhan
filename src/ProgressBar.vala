public class Daikhan.ProgressBar : Gtk.Scale {
    ...
    construct {
        ...
        player.notify["state"].connect (update_sensitivity);
    ...
    void update_sensitivity () {
        sensitive = player.state >= Daikhan.PlaybinProxy.State.BUFFERING;
    }

    double last_progress_change = 0;

    bool change_value_cb (Gtk.Range range, Gtk.ScrollType scrl_type, double value) {
        if (last_progress_change == value || value < 0 || player.state < Daikhan.PlaybinProxy.State.BUFFERING) {
            return true;
        }
